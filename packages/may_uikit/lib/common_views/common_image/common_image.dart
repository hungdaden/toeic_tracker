...
import 'asset_gen_image.dart';
import 'svg_gen_image.dart';
import '../common_progress_indicator/common_progress_indicator.dart';
import '../../utils/extenstion.dart';
import '../../may_uikit.dart';

/// Unified image widget that supports multiple image sources and fallback states.
class CommonImageV2 extends StatelessWidget {
  const CommonImageV2._({
        ...
        if (imageUrl.toLowerCase().contains('.svg')) {
          image = _svg.SvgPicture.network(
            ...
            placeholderBuilder: (context) {
              if (_style.placeholderBuilder != null) {
                return _style.placeholderBuilder!(context);
              }
              return _showProgressIndicator(_style);
            },
            errorBuilder: (context, error, stackTrace) {
              if (_style.errorBuilder != null) {
                return _style.errorBuilder!(context, error);
              }
              return const SizedBox.shrink();
            },
            colorFilter: _style.foregroundColor?.let(
              (it) => ColorFilter.mode(it, _style.colorBlendMode ?? BlendMode.srcIn),
            ),
          );
        } else if (_style.useCachedNetworkImage) {
          ...
          final memCacheWidth = _style.memCacheWidth ??
              (_style.autoResizeMemory && _style.width != null
                  ? maxWidth.times(devicePixelRatio).toInt()
                  : null);
          final memCacheHeight = _style.memCacheHeight ??
              (_style.autoResizeMemory && _style.height != null
                  ? maxHeight.times(devicePixelRatio).toInt()
                  : null);
...
/// Supported image source types for [CommonImageV2].
enum ImageInputType {
  svg,
  asset,
  network,
...
class _CommonNetworkImageStyle extends _CommonImageStyle {
  const _CommonNetworkImageStyle({
    ...
    required this.autoResizeMemory,
